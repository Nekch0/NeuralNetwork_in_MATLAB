classdef BatchNorm1D
    ...
    methods
        function status = BatchNorm1D(momentum)
        ...
        function status = set_IOSize(status, inputSize)
            ...
            status.gamma = ones([1 inputSize(end)]);
            status.beta  = zeros([1 inputSize(end)]);
            status.gamma_buffer = zeros(size(status.gamma));
            status.beta_buffer  = zeros(size(status.beta));
            status.running_var  = zeros([1 inputSize(end)]);
            status.running_mean = zeros([1 inputSize(end)]);            
        ...
        function [output, status] = forward(status, input, batch_size, eval_type)
            ...
            if isequal(eval_type, 'train')
                input_mean = mean(input, 1);
                status.input_dev = input(:, :, :, :) - input_mean(:, :, :, :);
                status.input_var = var(input, 0, 1);
        ...
        function [output, status] = backward(status, input, ~)
            status.beta_grad  = sum(input, 1);     
            status.gamma_grad = sum(input .* status.data_buffer_layer, 1); 
            inputSize_ = size(input);
            batch_     = inputSize_(1);
            input_hat_grad = input .* status.gamma;
            
            var_grad  = -sum(input_hat_grad .* status.input_dev, 1) ./ (2 * status.input_std.^3);
            mean_grad = -sum(input_hat_grad, 1) ./ status.input_std + var_grad .* (-2) .* sum(status.input_dev, 1) / batch_;
